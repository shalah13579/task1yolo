...
export default class EmployeeInternsList extends React.Component {
  ...
  componentDidMount() {
    axios.get(`http://localhost:3000/api/interns`)
      .then((res) => {
  ...
  deleteIntern(id, e) {
    axios.delete(`http://localhost:3000/employee/deleteemployee2`)
      .then((res) => {
        console.log(res);
        ...
        <table className="table-fixed w-full bg-white rounded-md shadow-lg">
          <thead className="bg-blue-400 text-white">
            <tr>
              ...
              <th className="w-2/12 p-2 border border-blue-400 text-left">DELETE</th>
            </tr>
          </thead>
          <tbody>
  ...
  <tr key={intern.id}>
    ...
    <td className="p-2 border border-blue-400 text-center">
      <button
        onClick={(e) => this.deleteIntern(intern.id, e)} // Pass intern.id as an argument
